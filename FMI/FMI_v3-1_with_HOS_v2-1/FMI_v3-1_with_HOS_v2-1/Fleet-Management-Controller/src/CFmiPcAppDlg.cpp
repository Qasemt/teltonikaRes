...
BEGIN_MESSAGE_MAP( CFmiPcAppDlg, CDialog )
    ...
    ON_BN_CLICKED( IDC_MAIN_BTN_FILE_TRANS, OnBnClickedFileTrans )
...
#endif
#if( FMI_SUPPORT_A609 )
    ON_UPDATE_COMMAND_UI( ID_FILE_REBOOTDEVICE, OnUpdateFileRebootDevice )
    ON_COMMAND( ID_FILE_REBOOTDEVICE, OnFileRebootDevice )
#endif
#if( FMI_SUPPORT_A610 )
    ON_UPDATE_COMMAND_UI( ID_FILE_AOBRDDRIVERS, OnUpdateFileAOBRDDrivers )
    ON_COMMAND( ID_FILE_AOBRDDRIVERS, OnFileAOBRDDrivers )
    ON_MESSAGE( WM_EVENT( EVENT_FMI_AOBRD_FILE_TRANSFER_RECEIVED ), OnEventAobrdFileTransferReceived )
    ON_BN_CLICKED(IDC_MAIN_BTN_SET_AOBRD_ODOMETER, OnBnClickedAobrdOdometer)
#endif
#if( FMI_SUPPORT_A611 )
    ON_MESSAGE( WM_EVENT( EVENT_FMI_LONG_TEXT_MSG_ERROR ), OnEventLongTextMsgError )
#endif
#if( FMI_SUPPORT_A612 )
    ON_COMMAND( ID_TOOLS_CUSTOMFORMS, OnToolsCustomForms )
#endif
#if( FMI_SUPPORT_A613 )
    ON_COMMAND( ID_TOOLS_MAP_VIEWER, OnToolsMapViewer )
#endif
#if( FMI_SUPPORT_A615 )
    ON_COMMAND( ID_TOOLS_IFTA, OnToolsIfta )
    ON_BN_CLICKED( IDC_MAIN_BTN_AUTO_STATUS_UPDATES, OnBnClickedAutoStatusUpdates )
    ON_COMMAND( ID_8HOURRULE_ENABLE, OnToolsHOS8HourRuleEnable )
    ON_COMMAND( ID_8HOURRULE_DISABLE, OnToolsHOS8HourRuleDisable )
#endif
#if( FMI_SUPPORT_A616 )
    ON_BN_CLICKED( IDC_MAIN_BTN_CHANGE_BAUD_RATE, OnBnClickedChangeBaudRate )
#endif
#if( FMI_SUPPORT_A617 )
    ON_BN_CLICKED( IDC_MAIN_BTN_SEND_ALERT, OnBnClickedSendAlert )
    ON_BN_CLICKED( IDC_MAIN_BTN_SEND_SENSOR, OnBnClickedSendSensor )
#endif
#if( FMI_SUPPORT_A619 )
    ON_COMMAND( ID_CONFIGURE_HOS_PERIODIC_UPDATES, OnToolsConfigureHosPeriodicUpdates )
#endif
...
#endif
#if FMI_SUPPORT_A612
    delete mCustomFormsDlg;
#endif
#if FMI_SUPPORT_A613
    delete mMapViewerDlg;
#endif
#if FMI_SUPPORT_A615
    delete mIftaDlg;
#endif
#if FMI_SUPPORT_A617
    delete mSendAlertDlg;
    delete mSendSensorDlg;
#endif
}
...
BOOL CFmiPcAppDlg::OnInitDialog()
{
    ...
    GetDlgItem( IDC_MAIN_BTN_FILE_TRANS )->EnableWindow( FALSE );
...
#if( !FMI_SUPPORT_A608 )
    GetDlgItem( IDC_MAIN_BTN_SPEED_LIMIT_ALERTS )->EnableWindow( FALSE );
#endif

#if( !FMI_SUPPORT_A610 )
    GetDlgItem( IDC_MAIN_BTN_SET_AOBRD_ODOMETER )->EnableWindow( FALSE );
    GetMenu()->GetSubMenu(0)->EnableMenuItem( ID_FILE_AOBRDDRIVERS, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
    GetMenu()->GetSubMenu(0)->EnableMenuItem( ID_FILE_REBOOTDEVICE, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
#endif

#if( !FMI_SUPPORT_A615 )
    GetDlgItem( IDC_MAIN_BTN_AUTO_STATUS_UPDATES )->EnableWindow( FALSE );
    GetMenu()->GetSubMenu( 1 )->GetSubMenu( 5 )->EnableMenuItem( 0 , MF_BYPOSITION | MF_DISABLED | MF_GRAYED );
#endif

#if( !FMI_SUPPORT_A616 )
    GetDlgItem( IDC_MAIN_BTN_CHANGE_BAUD_RATE )->EnableWindow( FALSE );
#endif

#if( !FMI_SUPPORT_A617 )
    GetDlgItem( IDC_MAIN_BTN_SEND_ALERT )->EnableWindow( FALSE );
    GetDlgItem( IDC_MAIN_BTN_SEND_SENSOR )->EnableWindow( FALSE );
#endif

#if( !FMI_SUPPORT_A619 )
    GetMenu()->GetSubMenu( 1 )->GetSubMenu( 5 )->EnableMenuItem( 1 , MF_BYPOSITION | MF_DISABLED | MF_GRAYED );
#endif
    ...
    case 607:
        ...
        break;
    case 608:
        this->SetWindowText( _T("Fleet Management Controller - A608 Support Mode") );
        break;
    case 610:
        this->SetWindowText( _T("Fleet Management Controller - A610 Support Mode") );
        break;
    case 611:
        this->SetWindowText( _T("Fleet Management Controller - A611 Support Mode") );
        break;
    case 612:
        this->SetWindowText( _T("Fleet Management Controller - A612 Support Mode") );
        break;
    case 613:
        this->SetWindowText( _T("Fleet Management Controller - A613 Support Mode") );
        break;
    case 614:
        this->SetWindowText( _T("Fleet Management Controller - A614 Support Mode") );
        break;
    case 615:
        this->SetWindowText( _T("Fleet Management Controller - A615 Support Mode") );
        break;
    case 616:
        this->SetWindowText( _T("Fleet Management Controller - A616 Support Mode") );
        break;
    case 617:
        this->SetWindowText( _T("Fleet Management Controller - A617 Support Mode") );
        break;
    case 618:
        this->SetWindowText( _T("Fleet Management Controller - A618 Support Mode") );
        break;
    case 619:
        this->SetWindowText( _T("Fleet Management Controller - A619 Support Mode") );
        break;
    default:
...
void CFmiPcAppDlg::OnBnClickedEnable()
{
...
#if( FMI_SUPPORT_A612 )
    if ( NULL == mCustomFormsDlg )
        {
        mCustomFormsDlg = new CCustomFormsDlg( this, mCom );
        mCustomFormsDlg->Create( CCustomFormsDlg::IDD );
        }
#endif
    ...
    if ( NULL == mIftaDlg )
        {
        mIftaDlg = new CIftaDlg( this, mCom );
        mIftaDlg->Create( CIftaDlg::IDD );
        }
#endif

#if( FMI_SUPPORT_A617 )
    if ( NULL == mSendAlertDlg )
        {
        mSendAlertDlg = new CSendAlertDlg( this, mCom );
        mSendAlertDlg->Create( CSendAlertDlg::IDD );
        }
    if ( NULL == mSendSensorDlg )
        {
        mSendSensorDlg = new CSendSensorDlg( this, mCom );
        mSendSensorDlg->Create( CSendSensorDlg::IDD );
        }
#endif
...
#if( FMI_SUPPORT_A616 )
    // by this point there should be no problem with comm rate resolution
    mResolvingCommRate = FALSE;
#endif
...
#if( FMI_SUPPORT_A609 )
//----------------------------------------------------------------------
//! \brief ON_UPDATE_COMMAND_UI handler for File > Reboot Device menu item.
//! \details Called by MFC just before the File menu is displayed to
//!     determine whether Reboot Device should be enabled or disabled.
//! \param aCmdUI Pointer to the CCmdUI object representing the File >
//!     Reboot Device menu item.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnUpdateFileRebootDevice
    (
    CCmdUI * aCmdUI
    )
{
...
//----------------------------------------------------------------------
//! \brief Handler for the File > Reboot Device.
//! \details Send a command to the attached device to reboot it.
//----------------------------------------------------------------------
...
#if( FMI_SUPPORT_A610 )
//----------------------------------------------------------------------
//! \brief ON_UPDATE_COMMAND_UI handler for File > AOBRD Drivers menu item.
//! \details Called by MFC just before the File menu is displayed to
//!     determine whether AOBRD Drivers should be enabled or disabled.
//! \param aCmdUI Pointer to the CCmdUI object representing the File >
//!     View AOBRD Drivers item.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnUpdateFileAOBRDDrivers
    (
    CCmdUI * aCmdUI
    )
{
    aCmdUI->Enable( mCom.mUseAOBRD );
}

//----------------------------------------------------------------------
//! \brief Handler for the File > AOBRD Drivers.
//! \details Show dialog for provisioning AOBRD driver profiles.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnFileAOBRDDrivers()
{
    CAobrdDriversDlg dlg( this, mCom );
    dlg.DoModal();
}

//----------------------------------------------------------------------
//! \brief Attempt to convert received file into human-readable CSV.
//! \details Attempt to convert received file into human-readable CSV.
//----------------------------------------------------------------------
afx_msg LRESULT CFmiPcAppDlg::OnEventAobrdFileTransferReceived( WPARAM, LPARAM )
{
    //convert the file to CSV
    AobrdEventLogConverter converter;
    converter.convertToCsv( SAVE_RECEIVED_AOBRD_LOG, SAVE_CONVERTED_AOBRD_LOG );
...
#if( FMI_SUPPORT_A615 )
//----------------------------------------------------------------------
//! \brief Handler for the Tools > IFTA.
//! \details Show dialog for managing IFTA data on device.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnToolsIfta()
{
    mIftaDlg->ShowWindow( SW_SHOW );
}
#endif
...
void CFmiPcAppDlg::initProductNames()
{
    ...
    mProductNames[ 1371 ] = CString("nuvi 2555");
    mProductNames[ 1389 ] = CString("nuvi 2585TV");
    mProductNames[ 1516 ] = CString("dezl 760 Series");
    mProductNames[ 1517 ] = CString("nuvi 27x7 Series");
    mProductNames[ 1582 ] = CString("nuvi 2xx7 Series");
    mProductNames[ 1758 ] = CString("nuvi 25x9 Series");
}
...
#if( FMI_SUPPORT_A616 )
        //in case the first packet fails, synchronize baud rate with device
        mRemainingRates.RemoveAll();
        //9600 is the default, so if that doesn't work initially, retry with the alternatives first
        mRemainingRates.Add( FMI_BAUD_RATE_57600 );
        mRemainingRates.Add( FMI_BAUD_RATE_9600 );

        mResolvingCommRate = TRUE;
#endif
        OnBnClickedEnable();
        mIgnoreTimer = FALSE;
...
void CFmiPcAppDlg::OnBnClickedSend()
{
    ...
    mCom.sendFreeformFmiPacket( fmiPacketId, fmiPayload, fmiPayloadSize );
}
...
//----------------------------------------------------------------------
//! \brief Click handler for the Transfer File button
...
{
    CFileTransferDlg dlg( this, mCom );
    dlg.DoModal();
}
...
#if( FMI_SUPPORT_A610 )
//---------------------------------------------------------------------
//! \brief Click handler for the AOBRD Set Odometer button
//! \details Display a modal dialog allowing the user to set
//!     AOBRD Odometer reading.
//! \since Protocol A610
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnBnClickedAobrdOdometer()
{
    CAobrdOdometerDlg dlg( this, mCom );
    dlg.DoModal();
}
#endif
...
#if( FMI_SUPPORT_A611 )
//----------------------------------------------------------------------
//! \brief Handler for the Server to Client Long Text Message Error
//!     event from Com.
//! \details Display an error message pop up.
//! \param aEventDataPtr The message ID.
//! \return 0, always
//! \since Protocol A611
//----------------------------------------------------------------------
afx_msg LRESULT CFmiPcAppDlg::OnEventLongTextMsgError
    (
    WPARAM aResultCode,
    LPARAM aEventDataPtr
    )
{
    ...
    switch( aResultCode )
    {
    case LONG_TEXT_INVALID_ID_SIZE:
        errorString.Format( _T("Text Message id %s is an invalid size!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_NON_ZERO_SEQ_NUM_ERROR:
        errorString.Format( _T("Text Message with id %s did not start its sequence numbers at zero!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_ID_IN_USE_SQL_ERROR:
        errorString.Format( _T("Text Message with id %s already exists!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_ID_IN_USE_ERROR:
        errorString.Format( _T("Text Message with id %s already exists!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_MALLOC_BUFFER_FAIL:
        errorString.Format( _T("Text Message with id %s could not be allocated memory!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_WRONG_ID_SIZE:
        errorString.Format( _T("Text Message id %s is an invalid size!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_WRONG_ID_ERROR:
        errorString.Format( _T("Text Message with id %s is the wrong id for long message continuation!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_NON_INC_SEQ_NUM_ERROR:
        errorString.Format( _T("Text Message with id %s did not increment its sequence numbers!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_WRONG_ORIG_TIME:
        errorString.Format( _T("Text Message with id %s did not have a matching origination time!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_WRONG_MESSAGE_TYPE:
        errorString.Format( _T("Text Message with id %s did not have a matching message type!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_EXCEEDED_MAX_BYTES:
        errorString.Format( _T("Text Message with id %s exceeds the maximum length!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_APPEND_NULL_PTR:
        errorString.Format( _T("Text Message with id %s had an early null terminator!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_SAVE_SQL_NULL_PTR:
        errorString.Format( _T("Text Message with id %s had an early null terminator!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_MALLOC_INBOX_FAIL:
        errorString.Format( _T("Text Message with id %s could not be allocated memory!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    case LONG_TEXT_SQL_INSERT_FAIL:
        errorString.Format( _T("Text Message with id %s could not be saved on the device's database!"), messageId->toCString( mCom.mClientCodepage ) );
        break;
    }

    AfxGetMainWnd()->MessageBox( errorString, _T("Long Text Message Failure" ) );
    delete messageId;
...
#if( FMI_SUPPORT_A612 )
//----------------------------------------------------------------------
//! \brief Handler for the Tools > Custom Forms action.
//! \details Show the Custom Forms library dialog.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnToolsCustomForms()
{
    mCustomFormsDlg->ShowWindow( SW_SHOW );
}
...
#if( FMI_SUPPORT_A613 )
//----------------------------------------------------------------------
//! \brief Handler for the Tools > Map Viewer action.
//! \details Show the Map Viewer dialog.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnToolsMapViewer()
{
     if ( mMapViewerDlg == NULL )
        {
        mMapViewerDlg = new CMapViewerDlg( this, mCom );
        mMapViewerDlg->Create( CMapViewerDlg::IDD );
        }
    mMapViewerDlg->ShowWindow( SW_SHOW );
}
...
#if( FMI_SUPPORT_A615 )
//----------------------------------------------------------------------
//! \brief Handler for the Auto Status Updates button.
//! \details Show the Auto Status Updates change dialog.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnBnClickedAutoStatusUpdates()
{
    BOOL enabled = FALSE;
    uint16 settingValue = 0;
    UINT result = CHosSettingDlg::CaptureSetting( this,
        IDS_HOS_AUTO_STATUS_UPDATES_TITLE,
        IDS_HOS_AUTO_STATUS_UPDATES_VALUE_LABEL,
        IDS_HOS_AUTO_STATUS_UPDATES_VALUE_UNIT,
        0, 65535,
        enabled, settingValue );
    if( IDOK == result )
    {
#if( FMI_SUPPORT_A619 )
        fmi_hos_set_settings_request_data_type setThreshold;
        memset( &setThreshold, 0, sizeof( setThreshold ) );
        setThreshold.tag = HOS_SETTING_AUTO_STATUS_STOP_MOVING_THRESHOLD_SECONDS;
        setThreshold.enable = enabled;
        setThreshold.settings_value = settingValue;
        mCom.sendFmiPacket( FMI_HOS_SET_SETTING_DATA_REQUEST, ( uint8* ) &setThreshold, sizeof( setThreshold ) );
#else
        fmi_hos_auto_status_update_feature message;
        memset( &message, 0, sizeof( message ) );
        message.enable = enabled;
        if( message.enable )
            {
            message.stop_moving_threshold = settingValue;
            }

        mCom.sendFmiPacket( FMI_HOS_AUTO_STATUS_FEATURE_REQUEST, (uint8*)&message, sizeof( message ) );
#endif
    }
}
...
#if( FMI_SUPPORT_A619 )
//----------------------------------------------------------------------
//! \brief Handler for the HOS Periodic Updates configuration button.
//! \details Show the HOS Settings Capture dialog with Periodic Updates.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnToolsConfigureHosPeriodicUpdates()
{
    BOOL enabled = FALSE;
    uint16 settingValue = 0;
    UINT result = CHosSettingDlg::CaptureSetting( this,
        IDS_HOS_REPORT_STATUS_UPDATES_TITLE,
        IDS_HOS_REPORT_STATUS_UPDATES_VALUE_LABEL,
        IDS_HOS_REPORT_STATUS_UPDATES_VALUE_UNIT,
        0, 65535,
        enabled, settingValue );
    if( IDOK == result )
    {
        fmi_hos_set_settings_request_data_type setThreshold;
        memset( &setThreshold, 0, sizeof( setThreshold ) );
        setThreshold.tag = HOS_SETTING_PERIODIC_STATUS_PERIOD_SECONDS;
        setThreshold.enable = enabled;
        setThreshold.settings_value = settingValue;
        mCom.sendFmiPacket( FMI_HOS_SET_SETTING_DATA_REQUEST, ( uint8* ) &setThreshold, sizeof( setThreshold ) );
    }
...
//----------------------------------------------------------------------
//! \brief Enable the HOS 8 Hour Rule on the device
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnToolsHOS8HourRuleEnable()
    {
    GetMenu()->GetSubMenu( 1 )->GetSubMenu( 5 )->CheckMenuRadioItem( ID_8HOURRULE_ENABLE, ID_8HOURRULE_DISABLE, ID_8HOURRULE_ENABLE, 0 );
#if( FMI_SUPPORT_A619 )
    fmi_hos_set_settings_request_data_type request;
    memset( &request, 0, sizeof( request ) );
    request.tag = HOS_SETTING_EIGHT_HOUR_RULE_ENABLE;
    request.enable = true;
    mCom.sendFmiPacket( FMI_HOS_SET_SETTING_DATA_REQUEST, ( uint8* ) &request, sizeof( request ) );
    return;
#else
    boolean hos8HourRuleEnabled = true;
    mCom.sendFmiPacket( FMI_HOS_8_HOUR_RULE_ENABLE_REQUEST, ( uint8* ) &hos8HourRuleEnabled, sizeof( boolean ) );
#endif
    }

//----------------------------------------------------------------------
//! \brief Disable the HOS 8 Hour Rule on the device
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnToolsHOS8HourRuleDisable()
    {
    GetMenu()->GetSubMenu( 1 )->GetSubMenu( 5 )->CheckMenuRadioItem( ID_8HOURRULE_ENABLE, ID_8HOURRULE_DISABLE, ID_8HOURRULE_DISABLE, 0 );
#if( FMI_SUPPORT_A619 )
    fmi_hos_set_settings_request_data_type request;
    memset( &request, 0, sizeof( request ) );
    request.tag = HOS_SETTING_EIGHT_HOUR_RULE_ENABLE;
    ...
    return;
#else
    boolean hos8HourRuleEnabled = false;
    mCom.sendFmiPacket( FMI_HOS_8_HOUR_RULE_ENABLE_REQUEST, ( uint8* ) &hos8HourRuleEnabled, sizeof( boolean ) );
#endif
    }
#endif
...
#if( FMI_SUPPORT_A616 )
//----------------------------------------------------------------------
//! \brief Handler for the Change Baud Rate button.
//! \details Show the Baud Rate change dialog.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnBnClickedChangeBaudRate()
{
    CChangeBaudRateDlg dlg( this, mCom );
    dlg.DoModal();
}

//----------------------------------------------------------------------
//! \brief Attempt to communicate with the device using next baud rate
//! \details When initial communication times out, it may be because the
//!    device is operating at a different baud rate, so try until there
//!    are no more rates to try, at which point an error is reported
//----------------------------------------------------------------------
bool CFmiPcAppDlg::tryNextCommRate()
    {
    if( 0 == mRemainingRates.GetCount() )
        {
        mResolvingCommRate = FALSE;
        // this should only be reached after sync attempts for all supported rates result in timeout
        ::MessageBox( m_hWnd, _T( "Unable to establish communication with any of the supported BAUD rates" ), _T("Error"), MB_OK | MB_ICONERROR );
        if( !selectComPort())
            {
            AfxGetMainWnd()->SendMessage( WM_CLOSE );
            return false;
        ...
        // attempt to establish communication at current rate option
        // if that fails, a timeout will return control here to try the next rate
        switch( mRemainingRates[ 0 ] )
            {
        ...
        mRemainingRates.RemoveAt( 0 );

        // attempt to transmit the enable packet again
        mCom.clearError();
        }
...
#if( FMI_SUPPORT_A617 )
//----------------------------------------------------------------------
//! \brief Handler for the Send Alert button.
//! \details Show the Send Alert dialog.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnBnClickedSendAlert()
{
    mSendAlertDlg->reset();
    mSendAlertDlg->ShowWindow( SW_SHOW );
}
//----------------------------------------------------------------------
//! \brief Handler for the Add/Delete Sensor button.
//! \details Show the Add/Delete Sensor dialog.
//----------------------------------------------------------------------
void CFmiPcAppDlg::OnBnClickedSendSensor()
{
    mSendSensorDlg->reset();
    mSendSensorDlg->ShowWindow( SW_SHOW );
}
#endif