...
//!     a file transfer, a file data packet is not sent until the
...
public:
    ...
    public:
        ...
        bool sendCannedResponseTextMessage
            (
            const char   * aMessageText,
            const MessageId & aMessageId,
        ...
        //! \brief file version (FMI_GPI_FILE_INFORMATION)
        //! \since Protocol A604
        uint8                        mFileVersion[16];
        //! \brief Number of significant bytes in mFileVersion
        //! \since Protocol A604
        uint8                        mFileVersionLength;

        //! \brief file size (FMI_GPI_FILE_INFORMATION)
        //! \since Protocol A604
        uint32                       mFileSize;
        //! \brief Number of bytes of the file that have been transferred so far
        //! \since Protocol A604
        uint32                       mFileTransferBytesDone;
        //! \brief Total number of bytes in the file being transferred
        //! \since Protocol A604
        ...
        //! Flag to indicate that the user wants to stop the
        //!     file transfer
        //! \details Processed when the next file data receipt
        //!     is received.
        //! \since Protocol A604
    ...
    private:
...
#if( FMI_SUPPORT_A610 )
        void sendNextAOBRDShipment
            (
            char * driver_id
            );

        void sendNextAOBRDAnnotation
            (
            char * driver_id
            );
        ...
        void FmiApplicationLayer::startFileReceive
            (
            const file_info_data_type & file_size
            );
