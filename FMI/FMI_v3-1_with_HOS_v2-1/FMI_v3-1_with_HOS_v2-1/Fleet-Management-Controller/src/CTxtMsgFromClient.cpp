    ...
    )
{
    ...
    DDX_Text( aDataExchange, IDC_CLIENTMSG_TXT_LON, mLongitude );
    DDX_Text( aDataExchange, IDC_CLIENTMSG_TXT_LAT, mLatitude );
#endif
}
...
BOOL CTxtMsgFromClient::OnInitDialog()
{
    ...
    UTIL_format_time_string( &localDateTime, localTimeString, sizeof( localTimeString ) );
    ...
    if( mTextMessageEvent.latitude != INVALID_LAT )
    {
        double longitudeDegrees = UTIL_convert_semicircles_to_degrees( mTextMessageEvent.longitude );
        double latitudeDegrees = UTIL_convert_semicircles_to_degrees( mTextMessageEvent.latitude );

        if( latitudeDegrees < 0 )
        {
            mLatitude.Format(  _T( "%08.6f °S" ), latitudeDegrees * -1 );
        }
        else
        {
            mLatitude.Format(  _T( "%08.6f °N" ), latitudeDegrees );
        }

        if( longitudeDegrees < 0 )
        {
            mLongitude.Format(  _T( "%08.6f °W" ), longitudeDegrees * -1 );
        }
        else
        {
            mLongitude.Format(  _T( "%08.6f °E" ), longitudeDegrees );
        }
    }
