...
bool GarminLinkLayer::getPacketFromFifo()
{
    ...
    while( mFifo.head != mFifo.tail )
    {
        ...
        packet->mFrameSize = 0;
        uint32 endOfFrame = (mFifo.head + SIZE_OF_FOOTER) % FIFO_SIZE;
        while( prevHead != endOfFrame )
        {
        ...
        if( packet->mChecksum != 0 )
        {
            ...
            if( mTransportLayer )
            {
                uint8 payload[2] = { packet->mPacketId, 0 };
                mTransportLayer->tx
                    (
                    new GarminPacket( ID_NAK_BYTE, payload, sizeof( payload ) ),
                    TRUE
                    );
            }
            delete packet;
        ...
        else
        {
            if( mTransportLayer )
            {
                ...
                {
                    uint8 payload[2] = { packet->mPacketId, 0 };
                    mTransportLayer->tx( new GarminPacket( ID_ACK_BYTE, payload, sizeof( payload ) ), TRUE );
                }
