...
//----------------------------------------------------------------------
//! \brief Converts a time_type from local to UTC time.
//! \param[in] aUtcTime The UTC time as a time_type.
//! \param[out] aLocalTime The local time as a time_type.
//----------------------------------------------------------------------
void UTIL_convert_local_to_UTC
    (
    const time_type * aLocalTime,
    time_type       * aUtcTime
    )
{
    TIME_ZONE_INFORMATION tzinfo;
    ...
    {
    case TIME_ZONE_ID_DAYLIGHT:
        *aUtcTime = *aLocalTime + ( ( tzinfo.Bias + tzinfo.DaylightBias ) * SECONDS_PER_MINUTE );
        break;
    default:
        *aUtcTime = *aLocalTime + ( ( tzinfo.Bias + tzinfo.StandardBias ) * SECONDS_PER_MINUTE );
    }
}
    ...
    )
{
    ...
    if( morning )
        sprintf_s( aResultString, aResultStringSize, "%02d:%02d:%02d AM", hour, aDateTime->time.minute, aDateTime->time.second );
    else
        sprintf_s( aResultString, aResultStringSize, "%02d:%02d:%02d PM", hour, aDateTime->time.minute, aDateTime->time.second );
}
...
time_type UTIL_get_current_garmin_time()
{
    ...
    gmtime_s( &aCurrentTm, &aCurrentTime );

    return UTIL_get_garmin_time( aCurrentTm );
}
...
time_type UTIL_get_garmin_time( tm aTimeTm )
{
    ...
    time_type seconds = ( ( aTimeTm.tm_year - 90 ) * SECONDS_PER_DAY * DAYS_IN_1_YEAR )
                      + ( aTimeTm.tm_yday          * SECONDS_PER_DAY    )
                      + ( aTimeTm.tm_hour          * SECONDS_PER_HOUR   )
                      + ( aTimeTm.tm_min           * SECONDS_PER_MINUTE )
                      + ( aTimeTm.tm_sec                                );
    ...
    int leapDays = ( aTimeTm.tm_year - 88 ) / 4;

    // if in a leap year and on or before Feb 28
    //   -> leap day has not happened this year
    if( ( aTimeTm.tm_year % 4 == 0 ) &&
        ( (uint32) aTimeTm.tm_yday <= sYearDays[2] ) )
    {
        leapDays--;
    }
...
{
    sprintf_s( aResultString, aResultStringSize, "%02d/%02d/%04d", aDateTime->date.month, aDateTime->date.day, aDateTime->date.year );
}
    ...
    )
{
    ...
    {
        strcpy_s( aCardinalDirection, aCardinalDirectionSize, "" );
        return;
    }
...
//----------------------------------------------------------------------
//! \brief This function will uncompress a file into a new file
//! \details If the original file was not gz compressed then the result
//!          file will be an exact copy of the original
//----------------------------------------------------------------------
CString unzip( char * original )
    {
    gzFile zipFile = gzopen( original, "rb" );
    if ( NULL != zipFile)
        {
        CString newFilename( original );
        newFilename += _T( ".inflated" );
        CFile newFile;
        if ( newFile.Open( newFilename, CFile::modeCreate | CFile::modeWrite ) )
            {
            BYTE buf[1024];
            int len = 0;
            while ( ( len = gzread( zipFile, buf, 1024 ) ) > 0 )
                {
                newFile.Write( ( const void * ) buf, ( UINT ) len );
                }
            gzclose( zipFile );
            newFile.Close();
            return newFilename;
    ...
    return CString( original );
    }

//----------------------------------------------------------------------
//! \brief This function will uncompress a file into a specified file
//! \details If the original file was not gz compressed then the result
//!          file will be an exact copy of the original
//----------------------------------------------------------------------
BOOL unzip( char * original, LPCTSTR destination )
    {
    // if file can be unzipped, do that
    gzFile zipFile = gzopen( original, "rb" );
    if ( NULL != zipFile)
        {
        CFile newFile;
        if ( newFile.Open( destination, CFile::modeCreate | CFile::modeWrite ) )
            {
            BYTE buf[1024];
            int len = 0;
            while ( ( len = gzread( zipFile, buf, 1024 ) ) > 0 )
    ...
    return CopyFile( CString( original ), destination, FALSE );
    }